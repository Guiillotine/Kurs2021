...
System::Void Kurs2021::Exp2::buttonOk_Click(System::Object^ sender, System::EventArgs^ e)
{
    ...
    else if (((Convert::ToInt32(this->from->Text) > tablePKD.GetRowsNum()) || (Convert::ToInt32(this->from->Text) == 0)) && f) { MessageBox::Show("Строки с данным номером не существует", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error); f = 0; }
    ...
    else if (((Convert::ToInt32(this->to->Text) > tablePKD.GetRowsNum()) || (Convert::ToInt32(this->to->Text) == 0)) && f) { MessageBox::Show("Строки с данным номером не существует", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error); f = 0; }
    ...
    if (f)
    {
        if (tablePKD.ExpBigTable(fileName, from, to) == 0) MessageBox::Show("Не удалось открыть файл помещения в него экспортируемой таблицы", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
        else
        {
